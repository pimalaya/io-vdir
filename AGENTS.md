...
- The crate is unconditionally `no_std`; `std` is gated on the `client` feature, which owns every filesystem call. Coroutines stay I/O-free.
- The vdir format is specified at vdirsyncer.pimutils.org/en/stable/vdir.html. Cite it when behaviour follows from it.
